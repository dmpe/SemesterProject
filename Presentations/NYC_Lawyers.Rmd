...
head(data.NYS[grep("pricewaterhoecoopers", data.NYS$Comp.Name, ignore.case=T), 1:5]) # FAIL
...
patternsToLookFor <- c("corporation.", "corp", "corp.", "group", "pllc", "p.a.", "llp","l.p.", "llc", "l.l.c.", "l.l.p.", "ltd.","inc.", "inc", "us", "p.c.", "pc", "plc")
patTTT <- paste(patternsToLookFor, collapse = '|')


# Just an idea how to proceed

# checkAndCleanFormatting <- function(x) {
#   x <- stri_trim_both(x)  
#   for(i in 1:length(x)) {
#     if(stri_detect_regex(x[i],patTTT) == TRUE) {
#       # print("yes, the string does contain some of patterns above")
...
```{r}
apple <- com_name[grep("davis polk", com_name$Var1, ignore.case=T),]
