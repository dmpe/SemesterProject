...
Then I create a bar plot with TOP 20 names of companies and their number of lawyers registered in New York City. Beware that not all cases are handled by the function above, e.g. there is `davis polk and wardwell london` which however is NOT counted as (/assigned to) `davis polk and wardwell`!!! Thus the number are not exact. See below for a prime example.
...
com_name.plot <- com_name.plot + geom_bar(stat = "identity") + theme(
...
```{r}
dvspw <- com_name[grep("citi", com_name$Var1, ignore.case=T),]
head(dvspw, 10)
```
