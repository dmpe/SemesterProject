...
## Goal
Look at the US New York City Attorney Registrations for interesting informations. 
...
All of that is done by running a function using other functions of `stringi` (and `stringr` too) library. Even though it uses C behind it, it is still very slow (with some optimalisations too - e.g. sapply) and even requires to be run twice because of the pattern which is complicated and not bullet-proof.
